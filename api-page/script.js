...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const populatePageContent = () => {
        ...
        const creator = settings.apiSettings?.creator || 'DGXO';

        setPageContent(DOM.pageTitle, settings.name, "DGXO REST API");
        setPageContent(DOM.wm, `© ${currentYear} ${creator}. Semua hak dilindungi.`);
        setPageContent(DOM.appName, settings.name, "DGXO REST API");
        setPageContent(DOM.sideNavName, settings.name || "API");
        setPageContent(DOM.versionBadge, settings.version, "v3.5");
        setPageContent(DOM.versionHeaderBadge, settings.header?.status, "Aktif!");
        setPageContent(DOM.appDescription, settings.description, "Rest API yang simpel dan mudah digunakan");
        ...
        if (DOM.apiLinksContainer) {
            ...
            const defaultLinks = [{ url: "https://github.com/DGameGT/dgxorestapi", name: "Lihat di GitHub", icon: "fab fa-github" }];
